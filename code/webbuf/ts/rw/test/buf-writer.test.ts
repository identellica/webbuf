...
describe("BufWriter", () => {
  ...
  describe("writeUint8", () => {
    it("should write an unsigned 8-bit integer", () => {
      ...
      expect(result.bytes[0]).toEqual(u8.n);
    });
  });
  ...
  describe("varIntBufBigInt", () => {
    it("should write a bigint less than 253 as a single byte", () => {
      ...
      expect(result.bytes[0]).toBe(bn.n);
    });

    it("should write a bigint less than 0x10000 as a 3-byte integer", () => {
      const bn = U64BE.fromN(0xffff);
      const result = BufWriter.varIntU64BEBuf(bn);
      expect(result.bytes[0]).toBe(253);
      expect(
        result.bytes[1] &&
          result.bytes[2] &&
          (result.bytes[1] << 8) | result.bytes[2],
      ).toBe(bn.n);
    });
    ...
    it("should write a bigint greater than or equal to 0x100000000 as a 9-byte integer", () => {
      ...
      const readBn =
        (BigInt(result.bytes[1] ?? 0) << 56n) |
        (BigInt(result.bytes[2] ?? 0) << 48n) |
        (BigInt(result.bytes[3] ?? 0) << 40n) |
        (BigInt(result.bytes[4] ?? 0) << 32n) |
        (BigInt(result.bytes[5] ?? 0) << 24n) |
        (BigInt(result.bytes[6] ?? 0) << 16n) |
        (BigInt(result.bytes[7] ?? 0) << 8n) |
        BigInt(result.bytes[8] ?? 0);
