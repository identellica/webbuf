...
describe("Audit: Key derivation verification", () => {
  it("should derive key as SHA256(sharedSecret(alicePriv, bobPub))", () => {
    ...
    // Encrypt with acs2dh
    const acs2dhEncrypted = acs2dhEncrypt(
      alice.privKey,
      bob.pubKey,
      plaintext,
      iv,
...
describe("Audit: Third party cannot decrypt", () => {
  ...
  it("should not allow decryption with wrong private key", () => {
    ...
    expect(() =>
      acs2dhDecrypt(wrongKey.privKey, alice.pubKey, encrypted),
    ).toThrow();
  });
  ...
  it("should not allow decryption with wrong public key", () => {
    ...
    expect(() =>
      acs2dhDecrypt(bob.privKey, wrongKey.pubKey, encrypted),
    ).toThrow();
  });
});
...
describe("Audit: IV handling", () => {
  ...
  it("should generate random IV when not provided", () => {
    ...
    expect(acs2dhDecrypt(bob.privKey, alice.pubKey, encrypted1).toUtf8()).toBe(
      "test",
    );
    expect(acs2dhDecrypt(bob.privKey, alice.pubKey, encrypted2).toUtf8()).toBe(
      "test",
    );
  });
});
...
describe("Audit: Security properties", () => {
  it("should produce different ciphertext for same message to different recipients", () => {
    ...
    const encryptedForBob = acs2dhEncrypt(
      alice.privKey,
      bob.pubKey,
      plaintext,
      iv,
    );
    const encryptedForCharlie = acs2dhEncrypt(
      alice.privKey,
      charlie.pubKey,
      plaintext,
      iv,
