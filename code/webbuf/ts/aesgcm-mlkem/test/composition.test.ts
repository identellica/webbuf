...
it("preserves selected hybrid inputs, wire bytes and output ownership", () => {
  ...
  const inputs = [d.buf, z.buf, ek.buf, dk.buf, plaintext, aad, m.buf, iv.buf];
  const before = inputs.map(storage);
  const expected = _aesgcmMlkemEncryptDeterministic(
    ek.clone(),
    plaintext.clone(),
    m.clone(),
    iv.clone(),
  ...
  const recovered = aesgcmMlkemDecrypt(dk, input, aad);
  expect(recovered.toUtf8()).toBe("selected hybrid");
  expect(() =>
    aesgcmMlkemDecrypt(dk, input, WebBuf.fromUtf8("wrong aad")),
  ).toThrow();
