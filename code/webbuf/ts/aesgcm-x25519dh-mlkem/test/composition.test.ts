...
it("preserves selected hybrid inputs, wire bytes and output ownership", () => {
  ...
  const alicePub = fixed(x25519PublicKeyCreate(alice));
  const bobPub = fixed(x25519PublicKeyCreate(bob));
  ...
  const expected = _aesgcmX25519dhMlkemEncryptDeterministic(
    alice.clone(),
    bobPub.clone(),
    ek.clone(),
    plaintext.clone(),
  ...
  const recovered = aesgcmX25519dhMlkemDecrypt(bob, alicePub, dk, input, aad);
